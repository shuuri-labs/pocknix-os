...
| **DEFERRED** | **mangoapp / perf overlay** | Built `packages/mangohud` from source (provides mangoapp; pinned 0.8.3 = ROCKNIX's pairing w/ our gamescope). Added MANGOHUD_CONFIGFILE wiring; tried BOTH `gamescope --mangoapp` (ROCKNIX) and the separate-launch + stats-FIFO model (ChimeraOS gamescope-session-plus, in the committed launcher). mangoapp runs + Steam writes the config, but the overlay never composites. ROOT CAUSE: **mangoapp's glfw-Wayland init fails against gamescope's nested compositor** — `libdecor-cairo: Could not get required globals` → GLFW falls back to X11 (`Platform not initialized`) → mangoapp dies. glfw DOES have Wayland (dlopens libwayland). It's a version-coupled stack issue (mangoapp↔glfw↔gamescope↔libdecor/gtk3) the references avoid by building the whole stack matched. DEFER — revisit with the full SteamOS-session integration (steamos-manager / gamescope-session-plus) or a matched component set. NB: the in-game **MangoHud Vulkan layer** (`MANGOHUD=1`) is arch-specific, so for x86/FEX games the gamescope overlay is actually the right (arch-agnostic) tool — hence worth getting eventually. Committed launcher currently uses the separate-launch model (harmless: just skips mangoapp if it won't run). |
