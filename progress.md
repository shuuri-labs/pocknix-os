...
|---|---|---|
| **HIGH** | **D-pad + X/Y** (in progress) | Fixed via `pocknix-bsp` capability_map `rp6-gamepad.yaml` (id rp6_gamepad, from armada's ayn_mcu which matches the RP6): d-pad = BTN_DPAD_* (present, evtest-confirmed), X/Y swap = BTN_NORTH↔BTN_WEST. Switched target ds5→`deck` (Steam Deck pad). NB on-device test showed deck glyphs but NO input — likely the capability_map not loading; check `journalctl -u inputplumber`, fall back to `ds5` target to isolate. |
